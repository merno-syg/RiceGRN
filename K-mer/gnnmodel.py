        ...
        for i in range(num_layers):
            if i == 0:
                if conv_type == 'gcn':
                    ...
                    conv = TransformerConv(in_fea, hidden_channels, edge_dim=edge_dim, beta=True)
                ...
                if conv_type == 'gcn':
                    ...
                    conv = TransformerConv(hidden_channels, hidden_channels, edge_dim=edge_dim, beta=True)
                bn = torch.nn.BatchNorm1d(hidden_channels)
...
class DiffNet(torch.nn.Module):
    ...
    def forward(self, data, mask=None):
        if isinstance(data, Data):
            x, edge_index, edge_attr, batch = data.x, data.edge_index, data.edge_attr, data.batch
            x, mask = to_dense_batch(x, batch)[0].to(torch.float32), to_dense_batch(x, batch)[1].to(torch.float32)
